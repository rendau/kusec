...
const props = defineProps<{
  show: boolean
  /** Config map the imported items will belong to. */
  configmapId: string
  /** Existing items of the config map — used to detect key conflicts. */
  items: ConfigItemMain[]
}>()
...
async function submit(): Promise<void> {
  ...
  for (const row of previewRows.value) {
    ...
    try {
      if (row.existing) {
        ...
        await updateConfigItem(row.existing.id, {
          value: row.value,
          value_format: 'text',
          encoding: 'plain',
          file_name: '',
        ...
        await createConfigItem({
          configmap_id: props.configmapId,
          key: row.key,
          value: row.value,
          value_format: 'text',
          encoding: 'plain',
  ...
  message.success(`Config items imported: ${parts.join(', ')}`)
  close()
}
</script>
