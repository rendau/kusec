...
import { getConfigItem } from '@/api/configitem'
...
const props = defineProps<{
  show: boolean
  /** Id of the config item to display; details are (re)fetched when it changes. */
  itemId: string | null
}>()
...
<template>
  ...
  >
    <NDrawerContent title="Config item details" closable>
      <NSpin :show="loading">
        <template v-if="item">
          <NDescriptions :column="1" label-placement="top" bordered size="small">
            ...
            <NDescriptionsItem label="Config map">
              <NTag size="small" type="info">{{ nameOf(item.configmap_id) }}</NTag>
            </NDescriptionsItem>
