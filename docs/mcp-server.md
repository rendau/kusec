...
Ключи с полным доступом к API (`mcp_only=false`) выпускают **только админы** — не-админ
может создавать лишь `mcp_only=true` ключи и не может снять `mcp_only` со своего ключа
через Update (получит `no_permission`).
